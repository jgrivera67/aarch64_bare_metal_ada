...
package body Uart_Boot_Loader.Xmodem_Protocol is
   use System.Storage_Elements;

   Receive_Timeout_Usec : constant Interfaces.Unsigned_64 := 10_000_000;  --  10s
   ...
   procedure Receive_File (Load_Address : System.Address) is
      ...
      function Receive_Packet_Data (Load_Address : System.Address;
         ...
         Packet_Buffer : array (1 .. Packet_Size) of Interfaces.Unsigned_8 with
      ...
      begin
         for I in Packet_Buffer'Range loop
            Maybe_Received_Byte := Receive_Byte_With_Timeout (Receive_Timeout_Usec);
            if Maybe_Received_Byte.Valid then
               Packet_Buffer (I) := Maybe_Received_Byte.Byte;
               Computed_Checksum := @ + Packet_Buffer (I);
            else
               return Maybe_Checksum;
            end if;
         end loop;

         Maybe_Checksum := (Valid => True, Checksum => Computed_Checksum);
         return Maybe_Checksum;
      ...
      Received_Checksum : Interfaces.Unsigned_8 := 0;
   ...
   begin
      ...
      loop
         ...
         pragma Assert (Maybe_Computed_Checksum.Valid);
         Received_Checksum := Uart_Driver.Receive_Byte;
         if Received_Checksum /= Maybe_Computed_Checksum.Checksum then
            Uart_Driver.Put_Char (ASCII.NAK);
            goto Next_Iteration;
         end if;
...
end Uart_Boot_Loader.Xmodem_Protocol;
